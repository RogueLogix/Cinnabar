...
import static org.lwjgl.glfw.GLFWVulkan.nglfwGetRequiredInstanceExtensions;
...
public class VulkanCore implements Destroyable {
    ...
    private static Pair<VkInstance, Long> createVkInstance() {
        try (var stack = MemoryStack.stackPush()) {
            ...
            if (Cinnabar.CONFIG.EnableValidationLayers || Cinnabar.CONFIG.Debug) {
            ...
            } else if (!FMLLoader.isProduction() && System.getenv().get("ENABLE_VULKAN_RENDERDOC_CAPTURE") == null) {
                ...
                for (int i = 0; i < layerCount; i++) {
                    ...
                    // devenv, renderdoc isn't attached, and we have the mesa overlay, enable it
...
//            physicalDeviceFeatures10.sparseBinding(true);
    ...
    // tihs is a limitation of how modlauncher handles layers, there is no way around it
    @NativeType("int")
