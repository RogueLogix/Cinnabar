...
public record PointerWrapper(long pointer, long size) implements Comparable<PointerWrapper> {
    ...
    public void free() {
        if (this == NULLPTR) {
            // technically, safe to call free on a nullptr at the native level, but debug will complain if enabled
            return;
        }
        untrackPointer(this);
    ...
    public PointerWrapper putShortIdx(long index, short val) {
        return putShort(index * MagicMemorySizes.SHORT_BYTE_SIZE, val);
    }
    
    public PointerWrapper putIntIdx(long index, int val) {
        return putInt(index * MagicMemorySizes.INT_BYTE_SIZE, val);
    }
    
    public PointerWrapper putLongIdx(long index, long val) {
        return putLong(index * MagicMemorySizes.LONG_BYTE_SIZE, val);
    }
    
    public PointerWrapper putFloatIdx(long index, float val) {
        return putFloat(index * MagicMemorySizes.FLOAT_BYTE_SIZE, val);
    }
    
    public PointerWrapper putDoubleIdx(long index, double val) {
        return putDouble(index * MagicMemorySizes.DOUBLE_BYTE_SIZE, val);
    }
    
    public PointerWrapper putVector3iIdx(long index, Vector3ic vector) {
        return putVector3i(index * MagicMemorySizes.IVEC3_BYTE_SIZE, vector);
    }
    
    public PointerWrapper putVector4iIdx(long index, Vector4ic vector) {
        return putVector4i(index * MagicMemorySizes.IVEC4_BYTE_SIZE, vector);
    }
    
    public PointerWrapper putVector4fIdx(long index, Vector4fc vector) {
        return putVector4f(index * MagicMemorySizes.VEC4_BYTE_SIZE, vector);
    }
    
    public PointerWrapper putMatrix4fIdx(long index, Matrix4fc matrix4f) {
        return putMatrix4f(index * MagicMemorySizes.MATRIX_4F_BYTE_SIZE, matrix4f);
    }
    ...
    public short getShortIdx(long index) {
        return getShort(index * MagicMemorySizes.SHORT_BYTE_SIZE);
    }
    
    public int getIntIdx(long index) {
        return getInt(index * MagicMemorySizes.INT_BYTE_SIZE);
    }
    
    public long getLongIdx(long index) {
        return getLong(index * MagicMemorySizes.LONG_BYTE_SIZE);
    }
    
    public float getFloatIdx(long index) {
        return getFloat(index * MagicMemorySizes.FLOAT_BYTE_SIZE);
    }
    
    public double getDoubleIdx(long index) {
        return getDouble(index * MagicMemorySizes.DOUBLE_BYTE_SIZE);
    }
    
    public void getMatrix4fIdx(long index, Matrix4f matrix4f) {
        getMatrix4f(index * MagicMemorySizes.MATRIX_4F_BYTE_SIZE, matrix4f);
    }
