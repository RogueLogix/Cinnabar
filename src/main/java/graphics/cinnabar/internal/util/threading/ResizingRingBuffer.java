package graphics.cinnabar.internal.util.threading;
...
public final class ResizingRingBuffer<T> {
    ...
    @Deprecated(forRemoval = true)
    private final Condition notEmpty = waitingLock.newCondition();
    // only modified with mutex lock, no need for atomic
    @Deprecated(forRemoval = true)
